...
# Compute the statistical similarity of the first SMILES to a list of SMILES
#
...
if (defined($ARGV[1])) {
  ...
  print "Too few arguments given. Aborting\n" ;
  die ;
}
...
if (substr($option,0,1) eq "-") { # options requested
  ...
  for ($i=0 ; $i < length($option) ; $i++) {
    ...
    if (substr($option,$i,1) eq "m") {$matrix = 1}
  }
  $sumo = $verbose + $debug + $matrix ;
  ...
  if ($matrix > 0) {
    print "m atom assignment output\n" ;
  }
}
else {
  if (defined($ARGV[2])) {
...
#print "$query $mollist $MNAME\n" ;

if (-e "$query") {
  if ($verbose > 0) {     
    print "query SMILES $query found\n" ;
  }
}
else { 
  print "query SMILES $query not found\n" ;
  die ;
}

if (-e "$mollist") {
  if ($verbose > 0) {     
    print "list of other SMILES $mollist found\n" ;
  } 
}
else { 
  if ($mollist eq "xxxx") {
    print "No other SMILES given. Aborting\n" ; 
    die ;
  }
  print "list of other SMILES $mollist not found\n" ; 
  die ;
}
...
open(INPU,$query) or die "error opening $query : $!\n"; 
...
#
  ...
  print "query compound:\n" ;
  print "$smi $cid\n" ;

# determine information for query compound
#  
  smi2hin() ;  

# copy information of query compound
...
# $natoms     => $natq
# $typ[i]     => typq[i]   # not used
# $nusbst[i]  => $nsubstq[i]
# $nangles[i] => $nanglesq[i]
# $ntors[i]   => $ntorsq[i]
# $tn12[i][j] => $tnq12[i][j]
# $tn13[i][j] => $tnq13[i][j]
# $tn14[i][j] => $tnq14[i][j]
...
#   print "query atom $i nsubst $nsubstq[$i]\n" ;
#   print "query nangles $nanglesq[$i]\n" ;
#   print "query ntors $ntorsq[$i]\n" ;
  }
...
#     print "col0 $cols[0] col1 $cols[1] ttt $ttt\n" ;
...
# setup propability arrays for query compound
...
close (INPU) or die "error closing $query : $!\n";

# set up query compound finished

$ncomp = 0 ;
open(OTHERS,$mollist) or die "error opening $mollist : $!\n"; 

print "compounds to compare:\n" ;
print "SMILES name harmonic geometric arithmetic smaller logscaling\n" ;

while(defined($cj = <OTHERS>)) {
  @cols = split(/[\s\t]+/,$cj); # split at spaces or tabs
...
#
  if($smi =~ m/[cno]/) {
    print "$smi $cid lower case letters not allowed! Skipping\n" ;
    next ;
  }
...
#
  ...
  smi2hin() ;  
# information of this molecule is used directly 
# $natoms     # number of atoms
# $typ[i]     # atom type of atom i "NA"
# $nsubst[i]  # number of 1st neighbors of atom i (including hydrogens) = number of bonds
# $neib[i][j] # list of 1st neighbors of atom i
...
# setup propability arrays for compound
...
# third neighbors to third neighbors matrix
  for ($j=1 ; $j <= $natq ; $j++) {
    ...
    for ($k=1 ; $k <= $natoms ; $k++) {
      ...
      $s14[$j][$k] = ($sq14 + $sr14) / (2.0*max($sq14,$sr14)) ;
...
#     if ($matrix > 0) {print "matat $matat  query $ttj  reference $ttk $pas[$ttj][$ttk]\n"} 
    ...
    while ($matat < $natoms) {
      ...
      $matq[$ttk] = 1 ;
      $matr[$ttj] = 1 ;
#     if ($matrix > 0) {print "matat $matat  reference $ttj  query $ttk $pas[$ttk][$ttj]\n"} 
      $matat++ ;
    } # end while
  }
...
# similarity <= 1 
  ...
  $simtsm = $simt / $minn ; # devide by smaller molecule

  $simtar = (2.0 * $simt) / ($natq + $natoms) ; # arithmetic mean

  $simtge = $simt / sqrt($natq * $natoms) ; # geometric mean
 
  $simtha = 0.5 * $simt * (1.0 / $natq + 1.0 / $natoms) ; # harmonic mean

# $simtlg = $simt / ($minn + 0.5 * log($maxn - $minn + 1)) ; # log scaling
# produces too high values for less similar compounds
  
  $simtlg = $simt / ($minn + log($maxn - $minn + 1)) ; # log scaling

  printf ("%7.4f%7.4f%7.4f%7.4f%7.4f",$simtha,$simtge,$simtar,$simtsm,$simtlg) ;
  print "\n" ;

  if ($matrix > 0) { # print atom to atom assignment matrix query to reference
    if ($natoms > $natq) { # natoms rows
      print "     " ;
      for ($k=1 ; $k <= $natq ; $k++) {
        printf ("%3i",$k) ;
      }
      ...
      print "\n" ;
      for ($l=1 ; $l <= $natoms ; $l++) {
        printf ("%2i%4s",$l,$typ[$l]) ;
        for ($m=1 ; $m <= $natq ; $m++) {
          $ttt = $pas[$m][$l] ; # match score
          if ($ttt > 0.9999) {print " * "}
          elsif ($ttt > 0.90) {print " 9 "}
          elsif ($ttt > 0.80) {print " 8 "}
          elsif ($ttt > 0.70) {print " 7 "}
          elsif ($ttt > 0.60) {print " 6 "}
          elsif ($ttt > 0.50) {print " 5 "}
          elsif ($ttt > 0.40) {print " 4 "}
          elsif ($ttt > 0.30) {print " 3 "}
          elsif ($ttt > 0.20) {print " 2 "}
          elsif ($ttt > 0.10) {print " 1 "}
          elsif ($ttt > 0.01) {print " . "}
          else {print "   " }
        }
        print "\n" ;
      }
      ...
      print "\n" ;
      for ($l=1 ; $l <= $natq ; $l++) {
        printf ("%2i%4s",$l,$typq[$l]) ;
        for ($m=1 ; $m <= $natoms ; $m++) {
          $ttt = $pas[$l][$m] ; # match score
          if ($ttt > 0.9999) {print " * "}
          elsif ($ttt > 0.90) {print " 9 "}
          elsif ($ttt > 0.80) {print " 8 "}
          elsif ($ttt > 0.70) {print " 7 "}
